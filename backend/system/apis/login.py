...
from mall.utils.decimal_to_float import convert_decimals_to_floats
...
class SchemaOut(ModelSchema):
    ...
    avatars: str = Field(None, alias='avatars')
...
def login(request, data: LoginSchema):
    ...
    if user_obj:
        ...
        avatar = 'http://localhost:8088/media/' + str(user_obj_dic['avatars'])
        del user_obj_dic['password']
        del user_obj_dic['avatar']

        time_now = int(datetime.now().timestamp())
        # decimal to float
        user_obj_dic_float = convert_decimals_to_floats(user_obj_dic)
        jwt = FuJwt(SECRET_KEY, user_obj_dic_float, valid_to=time_now + TOKEN_LIFETIME)
        # 将生成的token加入缓存
        # cache.set(user_obj.id, jwt.encode())
        token = f"bearer {jwt.encode()}"
        user_obj_dic['avatars'] = avatar
        data = {
            "multi_depart": 1,
            "sysAllDictItems": "q",
            "departs": "e",
