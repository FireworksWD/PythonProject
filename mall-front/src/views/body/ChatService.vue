...
import { getMsgUserAPI, getCustomerAPI } from '@/api/message'
...
const reconnect = () => {
  ...
  if (ws.readyState !== WebSocket.OPEN) {
    ws = new WebSocket('ws://localhost:8088/ws/' + userStore.user.username)
  } else {
    return
  }
  ws.addEventListener('open', handlewsOpen, false)
...
const avatar = userStore.user.avatars
// const nickname = userStore.user.name
// console.log(userStore)
// console.log(userStore.user)
// 确保聊天室滚动到底部
const MsgList = ref([])
const thisCustomer = ref()
const isSend = ref(true)
watch(thisCustomer, (newValue) => {
  isSend.value = !newValue
})
const handleSelectUser = async (val) => {
  thisCustomer.value = val
  const res = await getMsgUserAPI(val)
  MsgList.value = []
...
const handleSendBtnClick = () => {
  let obj = {
    ...
    receive_user: thisCustomer.value,
    img: avatar
  }
  ws.send(
      JSON.stringify({
    ...
    <el-container class="layout-container-demo" style="height: 600px">
        ...
        <el-scrollbar>
          <el-menu
            ...
            <el-sub-menu index="1">
              ...
              <el-menu-item-group v-for="Item in customerList" :key="Item.username">
      ...
      <el-container>
            ...
            <el-col :span="10">
              <div class="toolbar" style="display: flex; align-items: center;">
                ...
                <div class="user-info" style="margin-left: 10px; display: flex; align-items: center;">
                  ...
                  <el-avatar size="default" fit="fit" v-if="avatar" :src="avatar"> {{
                      username
                    }}
                  </el-avatar>
        ...
        <el-main style="margin-top: 1px; overflow-y: hidden;background-color: #ffffff">
          <el-scrollbar class="chat-room" id="chat-room" ref="scrollbarRef" style="margin-top: 1px">
            <div v-for="item in MsgList" :key="item.uid" ref="innerRef" class="message-container">
              <!-- 左边 -->
              ...
              <div v-else style="margin-right: 10px; margin-bottom: 8px">
                <el-row class="row-bg" type="flex" justify="end" align="middle">
                  ...
                  <el-avatar size="default" fit="fit" :src="avatar"> {{ item.username }}</el-avatar>
                </el-row>
        ...
        <el-row class="row-bg" type="flex" justify="space-around" align="middle">
          <el-col :span="20">
            <el-input type="text" class="search-input" autofocus @keyup.enter="handleSendBtnClick"
                      placeholder="请输入消息按Enter发送" v-model="currentMsg"></el-input>
          </el-col>
          <el-col :span="2">
            <el-button :disabled="isSend" id="searchBtn" @click="handleSendBtnClick" type="primary" size="default">
              发送
            </el-button>
          </el-col>
        </el-row>
