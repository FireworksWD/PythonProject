...
const register = async () => {
  ...
  const res = await userRegisterAPI(formModel)
...
const login = async () => {
  ...
  if (res.data.code === 500) {
    ...
    userStore.setUserInfo(res.data.result.userInfo)
    await router.push('/')
  }
  if (remember.value) {
        ...
        <div class="login-box form">
          ...
          <el-form ref="form" :model="formModel" :rules="rules" size="large" autocomplete="on" v-else>
            ...
            <el-form-item prop="password">
              <el-input v-model="formModel.password" :prefix-icon="Lock" placeholder="请输入密码" name="password"
